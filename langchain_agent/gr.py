...
async def add_to_cart(items: list[str] = []):
    ...
    Search for "{items[0] if items else 'product'}" on Amazon.in at the home page.
    Wait for search results to load completely.

    STEP-BY-STEP PROCESS:
    1. First, use the extract action to list first 10 products on the page with their COMPLETE product URLs (full URL starting with https://), product names, prices, brands (if visible), sizes (if visible), and identify which ones are "Sponsored" and which are NOT sponsored.
    ...
    8. IMMEDIATELY after successfully adding to cart, use the done action with the extracted product details formatted as GroceryCart JSON. DO NOT perform any additional steps after adding to cart (no file reading, no verification, no extra extracts).
    ...
    - Once the product is added to cart, IMMEDIATELY use the done action to complete the task with the product details
    ...
    - DO NOT click the add to cart button again after it's already been clicked
